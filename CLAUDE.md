...
一項變更在標記為完成之前，須通過以下四項：

1. **瀏覽器手動測試**：實際開啟 `index.html`，走過 golden path 與邊界情況（例如：拒食 vs 沒餵、0 隻/多隻切換、圖表各種粒度）。專案目前沒有自動化測試框架，這一步不可省略。
2. **Code review**：用 `/code-review` 檢查正確性與可簡化之處。
3. **文件同步更新**：功能異動需同步更新 README（功能說明、Roadmap/版本號）與 [TODO.md](TODO.md)（勾掉已完成項目）。
4. **資料相容性檢查**：見下方「資料結構變更政策」。
